...
VOID rpb_check(uint64_t addr, CONTEXT * ctxt, Block &i, std::string disassins)
{
  if (addr > 0x700000000000)
    return;
  #ifdef DISASS_DEBUG
  std::cout<<std::hex<<addr<<"\t"<<disassins<<dec<<endl;
  #endif
  ...
  {
    #ifdef RPB_DEBUG
    std::cout <<  std::hex << "rbp: " << i.rbp_value << '\n';
    #endif
  }
...
// This sets up rbp value and sets the size of the stack for the corresponding block
...
// This sets up rsp value and sets the size of the stack for the corresponding block
VOID rsp_val_set(CONTEXT * ctxt, Block &i)
{
  ...
  for(std::unordered_map<std::string, ObjInfo*>::iterator iter = i.objinfostack.begin(); iter != i.objinfostack.end(); ++iter)
  {
    std::string k = iter->first;
    ObjInfo* v = iter->second;
    for (auto j = i.rsp_value + v->get_ub(); j < i.rsp_value + v->get_lb(); ++j)
    {
      if(relPosStack.find(j) == relPosStack.end())
      {
        relPosStack.insert(std::make_pair(j, k));
  ...
  if (i.objinfostack.find(owner) != i.objinfostack.end())
  {
      ...
      i.objinfostack[owner]->get_obj() == "scalar")
    {
      ...
      else
      {
        if(accessboundsmap.find(owner) == accessboundsmap.end())
          accessboundsmap.insert(std::make_pair(owner, new AccessBounds(i.objinfostack[owner]->get_lb() +
        i.rsp_value, i.objinfostack[owner]->get_ub() + i.rsp_value)));
      }
  ...
  std::string owner, int64_t displacement, REG base_reg, REG index_reg, int64_t scale, int64_t ins_size)
{
  ...
  if (i.objinfostack.find(owner) != i.objinfostack.end())
  {
    ...
    else if (i.objinfostack[owner]->get_obj() == "scalar" || i.objinfostack[owner]->get_obj() == "array")
    {
      if (i.adjust_off)
      {
        if(accessboundsmap.find(owner) == accessboundsmap.end())
...
VOID Instruction(INS ins, VOID *v)
{
  ...
  // if rsp relative addressing is used
  else
  {
    if (!i->rsp_set_flag)
    {
  ...
  && INS_OperandIsImmediate(ins, 1))
  {
    ...
    if (REG_StringShort(REG_FullRegName(INS_OperandMemoryBaseReg(ins, 0))) == "rbp" || REG_StringShort(REG_FullRegName(INS_OperandMemoryBaseReg(ins, 0))) == "rsp")
    {
