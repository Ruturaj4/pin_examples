  ...
  std::string owner, int64_t immediate, int64_t displacement, REG base_reg)
{
  ...
  if (i.objinfostack[owner]->get_obj() == "heap_pointer")
  {
    ...
    effective_dispacement < accessboundsmap[owner]->get_bound()))
    {
      std::cout << "Boundover heap accessed by " << owner << '\n';
      // std::exit(1);
    }
  }
}
  ...
  REG reg)
{
  ...
  // TODO: this check can be done at the begining of the function
  // and can be returned to avoid an overhead
  if (i.objinfostack[owner]->get_obj() == "heap_pointer")
  {
    ...
    if(accessboundsmap.find(owner) == accessboundsmap.end())
    {
      accessboundsmap.insert(std::make_pair(owner,
        new AccessBounds(i.objinfostack[owner]->get_size() + effective_dispacement,
...
VOID RecordMalloc(ADDRINT addrs)
{
  if (!key) return;
  if (addrs == '\0')
  {
  ...
  map<ADDRINT, bool>::iterator it = MallocMap.find(addrs);
  if (it != MallocMap.end())
  {
    if (it->second)
    {
  ...
  {
    MallocMap.insert(pair<ADDRINT, bool>(addrs, false));
  }
}

VOID RecordFree(ADDRINT addrs)
{
  if (!key) return;
  map<ADDRINT, bool>::iterator it = MallocMap.find(addrs);
  if (it != MallocMap.end())
  {
    if (it->second)
    {
      // Object has been freed more than once
      std::cout << "The object has been freed more than once" << '\n';
    }
    else
    {
  ...
  else
  {
    ...
    std::cout << "Freeing the unallocated memory at" << addrs << "." << '\n';
  }
}
  ...
  if (RTN_Valid(mallocRtn))
    {
        ...
        RTN_InsertCall(mallocRtn, IPOINT_BEFORE, (AFUNPTR)ReallocBefore,
